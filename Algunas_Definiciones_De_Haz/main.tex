...
\begin{document}
   ...
   Nótese que por \textit{(F1)} se tiene que el elemento $s\in \mathscr{F}(U)$ cuya existencia garantiza \textit{(F2)} es único. Una definición alternativa de haz elimina la condición \textit{(F1)} pero exige la unicidad del $s\in \mathscr{F}(U)$ en \textit{(F2)}:
